Yes. Since your server appears to be Amazon Linux/RHEL-based (you were using yum/dnf), you can completely remove Jenkins like this.

1. Stop Jenkins
sudo systemctl stop jenkins
2. Disable Jenkins from starting automatically
sudo systemctl disable jenkins
3. Remove Jenkins package

For Amazon Linux 2023 / RHEL-based systems:

sudo dnf remove jenkins -y

If you're using an older system with yum:

sudo yum remove jenkins -y
4. Remove Jenkins data

⚠️ This deletes your Jenkins jobs, configurations, credentials, plugins, build history, and workspace data.

sudo rm -rf /var/lib/jenkins
sudo rm -rf /var/cache/jenkins
sudo rm -rf /var/log/jenkins
5. Remove Jenkins repository
...
You should see that the Jenkins service/package is no longer installed.

If you want a complete clean reinstall

Since you're practicing Jenkins and have been working with Nexus + Maven + Git, I'd recommend removing Jenkins completely and then reinstalling it cleanly rather than troubleshooting a potentially messy installation.

If you want, I can give you the exact Amazon Linux 2023 commands to uninstall and reinstall Jenkins from scratch, including Java 21 and Jenkins repository setup.

Failed to connect to repository : Error performing git command: git ls-remote -h 
subbareddy551/jenkins-projectone.git HEAD

This error means Jenkins is unable to execute git ls-remote successfully against GitHub:

Failed to connect to repository
Error performing git command:
git ls-remote -h https://github.com/subbareddy551/jenkins-projectone.git HEAD

Since you recently reinstalled Jenkins, let's troubleshoot it in order.

1. Check Git on the server
...
git --version
2. Test GitHub directly from the server

Run this as the Jenkins user:

sudo -u jenkins git ls-remote https://github.com/subbareddy551/jenkins-projectone.git HEAD

If it works, you'll get something similar to:

a1b2c3d4...        HEAD

If it fails, copy the exact error message here.

3. Check internet connectivity
sudo -u jenkins curl -I https://github.com

You should receive an HTTP response such as:

HTTP/2 200
4. Check Jenkins Git configuration

Go to:

Jenkins → Manage Jenkins → Tools

Under Git installations, make sure Git is configured.
...
https://github.com/subbareddy551/jenkins-projectone.git

Try:

git ls-remote https://github.com/subbareddy551/jenkins-projectone.git HEAD
6. If the repository is private

If jenkins-projectone is private, Jenkins needs GitHub credentials.

Go to:

Jenkins → Manage Jenkins → Credentials → Global → Add Credentials

For GitHub HTTPS, use:

Kind: Username with password
Username: your GitHub username
Password: GitHub Personal Access Token

Then select those credentials in your Jenkins Git configuration.

Don't use your normal GitHub password; use a GitHub Personal Access Token.

Most useful test

Please run these three commands on your server and send me the output:

git --version
sudo -u jenkins git ls-remote https://github.com/subbareddy551/jenkins-projectone.git HEAD
sudo -u jenkins curl -I https://github.com

The second command is the key one—it will tell us whether this is a GitHub authentication problem, network problem, or Jenkins/Git configuration problem.

Pasted text.txt
Document
make the UI with more realsistic model


Analyzed

I upgraded your existing UI to make it feel more like a real modern e-commerce website rather than a demo:

More realistic storefront hero section
Modern neutral color palette with orange accent
Better product cards, hover effects, spacing and shadows
More realistic category cards
Improved flash-sale section
Responsive mobile layout
Better search styling
Added cart “toast” feedback
Fixed malformed <emnput> and <emmg> HTML tags in your original file
Kept your existing products, categories and JavaScript functionality

Your original source was the basis for the changes.

Download the updated NexusShop UI 

NexusShop_realistic_UI.html
HTML
...
<body> <header> <div class="container header-inner" role="banner"> <div style="display:flex;align-items:center;gap:18px;"> <button class="mobile-toggle" id="mobileToggle" aria-label="Open menu"><em class="fas fa-bars"></em></button> <a class="brand" href="#"> <span>Nexus<span class="accent">Shop</span></span> </a> </div>
